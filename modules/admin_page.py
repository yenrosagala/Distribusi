...
def compute_upload_stats(table_type, df):
  ...
  stats = {
      "jumlah_baris": len(df),
      "jumlah_provinsi": (
          df["nama_provinsi"].nunique() if "nama_provinsi" in df.columns else 0
      ),
  }
  if table_type == "transportasi_udara":
    stats["total_penumpang"] = (
        df["penumpang_berangkat"].sum() + df["penumpang_datang"].sum()
    )
    stats["total_barang"] = df["barang_muat_kg"].sum() + df[
        "barang_bongkar_kg"
    ].sum()
    ...
    stats["total_penumpang"] = (
        df["dn_penumpang_naik"].sum() + df["dn_penumpang_turun"].sum()
    )
    stats["total_barang"] = df["dn_muat_barang_ton"].sum() + df[
        "dn_bongkar_barang_ton"
    ].sum()
...
def count_existing_rows(engine, table_type, tahun, bulan):
  try:
    result = pd.read_sql(
        text(
            f"SELECT COUNT(*) as n FROM {table_type} WHERE CAST(tahun AS TEXT)"
            " = :tahun AND bulan = :bulan"
        ),
        engine,
        params={"tahun": str(tahun), "bulan": bulan},
    )
...
def get_entity_options(engine, table, entity_col, kabupaten):
  ...
  if kabupaten != "SEMUA":
    kab_clean = (
        kabupaten.replace("KABUPATEN ", "").replace("KOTA ", "").strip()
    )
    query += (
        " WHERE (UPPER(nama_kabkota) = :kab_full OR UPPER(nama_kabkota) ="
        " :kab_clean)"
    )
...
def show_series_chart_section():
  ...
  table_map = {
      "Transportasi Udara": "transportasi_udara",
      "Transportasi Laut": "transportasi_laut",
  }
  entity_col_map = {
      "Transportasi Udara": "nama_bandara",
      "Transportasi Laut": "nama_pelabuhan",
  }
  entity_label_map = {"Transportasi Udara": "Bandara", "Transportasi Laut": "Pelabuhan"}

  st.markdown("### 📊 Filter Analisis Tren")
  with st.container():
  ...
  if st.button(
    ...
    if kabupaten != "SEMUA":
      ...
      conditions.append(
          "(UPPER(nama_kabkota) = :kab_full OR UPPER(nama_kabkota) = :kab_clean)"
      )
      params["kab_full"] = kabupaten.upper()
      params["kab_clean"] = kab_clean.upper()
    if entities_selected:
      placeholders = ", ".join(
          f":ent{i}" for i in range(len(entities_selected))
      )
    ...
    if df.empty:
      st.warning(
          "⚠️ Tidak ditemukan data yang cocok dengan kombinasi filter Anda."
      )
      return
    ...
    judul_variabel = ", ".join(
        v.replace("_", " ").title() for v in variabel_list
    )
...
def show_series_admin_page():
  st.markdown(
      "## 📈 Pusat Analisis & Manajemen Data"
  )
  st.markdown(
      "Jelajahi tren data transportasi antar periode secara bebas. Untuk"
      " melakukan perubahan data atau pengunggahan berkas baru, silakan gunakan"
      " panel autentikasi admin di bawah."
  )
  st.divider()
  ...
  # ==========================================================================
  # ADMIN PANEL SECTION (Organized with Tabs for cleaner UX)
  # ==========================================================================
  st.markdown("## 🔐 Portal Administrasi Sistem")
  ...
  st.success(
      "🔓 Anda berhasil masuk sebagai **Administrator**. Gunakan tab di bawah"
      " untuk mengelola sistem."
  )

  # Modern Tabbed Interface for Admin Actions
  admin_tab1, admin_tab2, admin_tab3 = st.tabs(
      ["📥 Ingest Data (Upload)", "🛠️ Koreksi Manual", "⚠️ Pengaturan & Hapus"]
  )

  # --- TAB 1: UPLOAD DATA ---
  with admin_tab1:
    st.markdown("### Unggah Data BPS Baru")
    st.caption(
        "Unggah satu atau beberapa berkas Excel sekaligus. Sistem akan"
    ...
    if uploaded_files:
      ...
      st.markdown("#### Langkah 1: Verifikasi Deteksi Otomatis")
        ...
        with st.container():
          ...
          with c1:
            ...
            if m["table_type"] == "transportasi_udara":
              st.caption("Moda: ✈️ **Transportasi Udara**")
            elif m["table_type"] == "transportasi_laut":
              st.caption("Moda: 🚢 **Transportasi Laut**")
            else:
              st.error("Format header tidak dikenali.")
          with c2:
            m["tahun"] = st.number_input(
                "Tahun",
                min_value=2020,
                max_value=2035,
                value=int(m["tahun"]),
      ...
      valid_meta = [
          m for m in st.session_state["upload_meta"] if m["table_type"] is not None
      ]

      if valid_meta and st.button(
          "🔍 Analisis & Pratinjau Berkas",
        ...
        for m in valid_meta:
          try:
            table_type, df_parsed = parse_transport_file(
                m["file_bytes"], m["tahun"], m["bulan"]
            )
            preview.append({
                ...
                "bulan": m["bulan"],
                "df": df_parsed,
                "stats": compute_upload_stats(table_type, df_parsed),
                "existing_rows": count_existing_rows(
                    engine, table_type, m["tahun"], m["bulan"]
                ),
                "error": None,
            })
      ...
      if "upload_preview" in st.session_state:
        ...
        st.markdown("#### Langkah 2: Ringkasan & Validasi Data")

        any_valid = False
        for p in preview:
          moda_label = (
              "✈️ Transportasi Udara"
              if p["table_type"] == "transportasi_udara"
              else (
                  "🚢 Transportasi Laut"
                  if p["table_type"] == "transportasi_laut"
                  else "⚠️ Gagal"
              )
          )
          with st.expander(
              f"{moda_label} — {p['nama_file']} ({p['bulan']} {p['tahun']})",
              expanded=True,
          ):
            if p["error"]:
            ...
            if p["existing_rows"] > 0:
              st.warning(
                  f"⚠️ Terdapat **{p['existing_rows']} baris data eksisting** untuk"
                  f" periode {p['bulan']} {p['tahun']}. Data lama akan ditimpa"
                  " jika opsi aktif."
              )
            else:
              st.info(
                  "✅ Periode ini bersih (belum ada data pada database)."
              )

            s = p["stats"]
            mcol = st.columns(4)
            mcol[0].metric("Jumlah Baris", f"{s['jumlah_baris']:,}")
            mcol[1].metric("Total Penumpang", f"{s['total_penumpang']:,.0f}")
            mcol[2].metric(
                "Total Barang", f"{s['total_barang']:,.2f} {s['satuan_barang']}"
            )
        ...
        if any_valid:
          ...
          overwrite = st.checkbox(
              "Timpa data lama jika periode sama sudah tersedia",
              value=True,
              key="upload_overwrite",
          )
          confirm = st.checkbox(
              "Saya telah memverifikasi statistik di atas dan siap menyimpannya"
              " ke database utama.",
              ...
              try:
                with engine.begin() as conn:
                  if overwrite:
                    conn.execute(
                        text(
                            f"DELETE FROM {p['table_type']} WHERE CAST(tahun AS"
                            " TEXT) = :tahun AND bulan = :bulan"
                        ),
                        {"tahun": str(p["tahun"]), "bulan": p["bulan"]},
                    )
                  p["df"].to_sql(
                      p["table_type"], conn, if_exists="append", index=False
                  )
                st.success(
                    f"✅ {p['nama_file']} berhasil disimpan ke"
                    f" `{p['table_type']}`."
                )
                success_count += 1
              except Exception as e:
      ...
      st.info("Unggah berkas untuk mulai memperbarui data.")

  # --- TAB 2: MANUAL CORRECTION ---
  with admin_tab2:
    st.markdown("### Koreksi Data Tabel Langsung")
    engine = get_engine()
    ...
    if st.button(
      ...
      df_to_edit = pd.read_sql(
          query, engine, params={"tahun": year_edit, "bulan": month_edit}
      )
      if df_to_edit.empty:
    ...
    if "df_to_edit" in st.session_state:
      ...
      if st.button(
        ...
        try:
          with engine.begin() as conn:
            del_query = text(
                f"DELETE FROM {table_edit} WHERE CAST(tahun AS TEXT) = :tahun"
                " AND bulan = :bulan"
            )
            conn.execute(
                del_query, {"tahun": year_edit, "bulan": month_edit}
            )
  ...
  # --- TAB 3: DANGER ZONE & DELETIONS ---
  with admin_tab3:
    st.markdown("### Zona Pemeliharaan & Penghapusan Data")
    st.markdown(
        "Hapus data berdasarkan kriteria filter tertentu untuk membersihkan"
        " entri yang salah atau duplikat."
    )
    ...
    with d1:
      ...
      table_del = (
          "transportasi_udara"
          if moda_del_label == "Transportasi Udara"
          else "transportasi_laut"
      )
    with d2:
    ...
    with d3:
      try:
        years_df = pd.read_sql(
            text(f"SELECT DISTINCT tahun FROM {table_del}"), engine_del
        )
        year_options = ["SEMUA"] + sorted(
            years_df["tahun"].astype(str).unique().tolist()
        )
      except Exception:
        year_options = ["SEMUA"]
    ...
    if jumlah_baris_del == 0:
      ...
      if not conditions:
        st.error(
            f"🚨 PERINGATAN: Tanpa filter aktif, **SEMUA {jumlah_baris_del}"
            f" baris** pada `{table_del}` akan dihapus!"
        )
      else:
        st.warning(
            f"⚠️ **{jumlah_baris_del} baris data** pada `{table_del}` akan"
            " dihapus permanen berdasarkan filter."
        )

      confirm_del = st.checkbox(
          "Saya paham tindakan ini permanen dan ingin melanjutkan.",
          key="confirm_delete_filtered",
      )
      if st.button(
          "🗑️ Hapus Data Sesuai Filter",
        ...
        try:
          with engine_del.begin() as conn:
            conn.execute(
                text(f"DELETE FROM {table_del} WHERE {where_clause}"),
                params_del,
            )
          st.success(
              f"✅ Berhasil menghapus {jumlah_baris_del} baris dari"
              f" `{table_del}`."
          )
    ...
    with st.expander("🚨 Aksi Darurat: Reset Total Basis Data"):
      st.error(
